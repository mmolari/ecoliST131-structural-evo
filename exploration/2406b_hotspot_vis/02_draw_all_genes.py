...
def load_data(hs):
    ...
    # load graph
    pan = pp.Pangraph.load_json(f"data/{hs}/joint_graph.json")
    ...
    # load block colors
    block_colors = pd.read_csv(f"res/{hs}/export/block_colours.csv", index_col=0)[
        "Colour"
    ].to_dict()

    return tree, gdf, pan, tdf, up, down, block_colors
