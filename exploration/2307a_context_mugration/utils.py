...
fig_fld = root / "exploration" / "2307a_context_mugration" / "figs"
...
def load_inference(context=False):
    loadfld = expl_fld / "mugration"
    fname = "infer_pa_context.json" if context else "infer_pa_simple.json"
    with open(loadfld / fname, "r") as f:
        data = json.load(f)
    return data
