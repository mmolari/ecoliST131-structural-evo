...
def country_barplot(df, svname, **kwargs):
    ...
    sns.despine()
    plt.tight_layout()
    plt.savefig(svfld / svname, **kwargs)
    plt.close(fig)
...
def metadata_tree(df_met, df_all, tree, svname_plot, svname_leg, **kwargs):
    ...
    plt.tight_layout()
    plt.subplots_adjust(wspace=0)
    plt.savefig(svfld / svname_plot, **kwargs)
    plt.close(fig)
    ...
    plt.savefig(svfld / svname_leg, **kwargs)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    # barplot by country
    country_barplot(df_met, "country_barplot.png", dpi=300)

    # collection date histogram
    collection_date_hist(df_met, "collection_date_hist.png", dpi=300)

    # metadata tree
    metadata_tree(
        df_met, df_all, tree, "metadata_tree.png", "metadata_tree_legend.png", dpi=300
    )
