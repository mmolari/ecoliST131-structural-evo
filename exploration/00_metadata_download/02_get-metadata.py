...
api_file = "../../config/ncbi_api_key.txt"
...
for i in range(len(id_dict.keys())):
    ...
    with open(tmp_file + ".xml", "w") as f:
        ...
        epost_process = subprocess.run(
            epost_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True
        )
        docsum_output = subprocess.run(
            ["efetch", "-format", "docsum"],
            input=epost_process.stdout,
            stdout=f,
