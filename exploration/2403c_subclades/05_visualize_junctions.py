...
fig_fld_main = pathlib.Path("figs/f05")
...
for dset in ["ST131_ABC", "ST131_sub_BC", "ST131_sub_C", "ST131_sub_C2"]:
    ...
    # junctions = []
    jdf = pd.read_csv(f"data/excluded_events/{dset}_n_events.csv", index_col=0)
    junctions = jdf.index.to_list()
    for junction in junctions:
        j_pos = jp[junction]
        graph_file = (
            fld / f"backbone_joints/asm20-100-5/joints_pangraph/{junction}.json"
        )
        ...
        plt.tight_layout()
        plt.savefig(fig_fld / f"{junction}.png", dpi=200)
        plt.show()
