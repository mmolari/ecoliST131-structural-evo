...
def parse_args():
    ...
    parser.add_argument("--plasmids_alleles", type=str)
    parser.add_argument("--plasmids_json", type=str)
    parser.add_argument("--resistance_pls", type=str)
    parser.add_argument("--resistance_chr", type=str)
...
def load_data(args):
    ...
    # metadata
    mdf = pd.read_csv(args.metadata, index_col=0)

    # MLST chromosome
    adf = pd.read_csv(args.alleles, index_col=0)

    # plasmid MLST
    pldf = parse_mlst_df(args.plasmids_alleles)

    with open(args.plasmids_json, "r") as f:
        plasmid_data = json.load(f)

    # resistance
    rdf_chr = pd.read_csv(args.resistance_chr, index_col=0, sep="\t")
    rdf_chr.index = rdf_chr.index.str.split("/").str[-1].str.removesuffix(".tab")
    rdf_chr = rdf_chr.drop(columns=["NUM_FOUND"])
    rdf_chr = rdf_chr.applymap(__convert)

    rdf_pld = pd.read_csv(args.resistance_pls, index_col=0)
...
def draw_tree(tree, strains, strain_y, mdf, adf, pldf, plasmid_data, rdf_chr, rdf_pld):
...
if __name__ == "__main__":
    ...
    tree, strains, strain_y, mdf, adf, pldf, plasmid_data, rdf_chr, rdf_pld = load_data(
        args
    )
    ...
    draw_tree(tree, strains, strain_y, mdf, adf, pldf, plasmid_data, rdf_chr, rdf_pld)
